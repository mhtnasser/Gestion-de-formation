...
if(isset($_POST['recup_submit'],$_POST['recup_mail'])) {
   if(!empty($_POST['recup_mail'])) {
      ...
      if(filter_var($recup_mail,FILTER_VALIDATE_EMAIL)) {
         $mailexist = $bdd->prepare('SELECT id, nom FROM emploie WHERE mail = ?');
         ...
         include('partial/content_mail.php');
         mail($recup_mail, "Récupération de mot de passe - M2L.com", $message, $header);
   ...
   if(isset($_POST['change_mdp'],$_POST['change_mdpc'])) {
      ...
      if($verif_confirme == 1) {
         ...
         if(!empty($mdp) AND !empty($mdpc)) {
            if($mdp == $mdpc) {
               $mdp = sha1($mdp);
               $ins_mdp = $bdd->prepare('UPDATE emploie SET mdp = ? WHERE mail = ?');
